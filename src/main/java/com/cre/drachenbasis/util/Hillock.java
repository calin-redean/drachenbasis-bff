...
{
	private static final Logger log = LoggerFactory.getLogger(Hillock.class);
    ...
    public Hillock( double[][] inputImageData, boolean[][] crit, double[][] sat, double del  )
    {
        ...
        //log.debug("HILLOCK: Min and Max of Hillock is " + minmax[0] + " and " + minmax[1]);
        setMaxRatio();
        stretch( threshold );
        setMaxRatio();
    }
    ...
    public int findFirstUnmarked()
    {
        ...
        for ( index = 0; index < numPixels; index++ )
            if ( marked[ index ] == UNMARKED )
                return index;
        log.debug("There is no unmarked pixel.");
        return -1;
    }
    ...
    public int[] findComponents()
    {
        ...
        while ( totalReached < target )
        {
            ...
            if ( start < 0 )
            {
                log.debug("There should be an unmarked node.");
                return null;
            }
        ...
        log.debug(" Number of components is " + numComponents );
